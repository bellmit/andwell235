package com.andawell.purchase.util.doc.poi;


import com.andawell.purchase.util.doc.WordReporter;
import com.andawell.purchase.util.doc.poi.bean.image.PoiImage;
import com.andawell.purchase.util.doc.poi.bean.table.IPoiWordTable;
import com.andawell.purchase.util.doc.poi.bean.table.PWATwithHeaderBottom;
import com.andawell.purchase.util.doc.poi.bean.table.PoiWordAutoTable;
...
public class PoiWordUtil {
     ...
     * 约定：
     * （1）文本：
     *      仅替换： @${t_xxx}@  （其中左右两边的@，可根据给定word文档不同的段落，自定义，但是其必须样式和周围不一致，其作为一个单独的XWPFRun
     * （2）表格：
     *        1. 静态表格（仅替换表格中的 占位文本）：
     ...
     */
    public static boolean changWord(final String inputUrl, String outputUrl,
...
//                                    final Map<String, List<IPoiWordTable>> autoTableRowMap,
        ...
        try {
            //获取docx解析对象
            // 解析成流
            ClassPathResource classPath = new ClassPathResource(inputUrl);
            ...
            //解析替换表格对象
            PoiWordUtil.changeTable(document, textMap, noneTableTextMap);
            //生成新的word
            ...
            wordReporter.setTempLocalPath(outputUrl);
            wordReporter.init();
            wordReporter.export(list, 2, 2, 3);
            wordReporter.generate(outputUrl);
        } catch (IOException e) {
            e.printStackTrace();
            changeFlag = false;
        }
     ...
     * @param document docx解析对象
     * @param textMap  需要替换的信息集合
     * @param noneTableTextMap  存储tablekey的容器
     */
...
//
            if (PoiWordKeyMatchRule.checkStaticTable(table.getRow(0).getCell(0).getText())) {
                // 获取表格头的key名
